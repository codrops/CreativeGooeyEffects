/*(function($){
	$.fn.tdown=function(handler){
		$(this).on("mousedown",function(event,pageX,pageY){
			event.pageX=pageX;
			event.pageY=pageY;
			handler(event);
		});
		$(this).on("touchstart",function(event){
			...
			$(this).trigger("mousedown",[touch.pageX,touch.pageY]);
			event.preventDefault();
			event.stopPropagation();
		});
	}

	$.fn.tmove=function(handler){
		$(this).on("mousemove",function(event,pageX,pageY){
			event.pageX=pageX;
			event.pageY=pageY;
			handler(event);
		});
		$(this).on("touchmove",function(event){
			var touch=event.originalEvent.touches[0];
			$(this).trigger("mousemove",[touch.pageX,touch.pageY]);
			event.preventDefault();
			event.stopPropagation();
		})
	}

	$.fn.tup=function(handler){
		$(this).on("mouseup",function(event,pageX,pageY){
			event.pageX=pageX;
			event.pageY=pageY;
			handler(event);
		});
		$(this).on("touchend",function(event){
			var touch=event.originalEvent.touches[0];
			$(this).trigger("mouseup",[touch.pageX,touch.pageY]);
			event.preventDefault();
			event.stopPropagation();
		});
	}
